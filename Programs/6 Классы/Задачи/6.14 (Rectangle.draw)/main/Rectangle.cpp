...
void Rectangle::setPerimeterCharacter(char p){ charPerimeter = p; }
void Rectangle::setFillCharacter(char f) { charFill = f; }

void Rectangle::draw()
{
	for (int i = 24; i >= 0; i--) {
		for (int j = 0; j < 25; j++)
			if ((i < y2 && i > y1) && (j > x1&& j < x4))
				cout << charFill;
			else
				if (((i == y1 || i == y2) && (j >= x1 && j <= x4)) || ((j == x1 || j == x4) && (i <= y2 && i >= y1)))
					cout << charPerimeter;
				else
					cout << " ";
		cout << endl;
